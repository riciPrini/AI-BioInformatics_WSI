...
### MCAT Implementation ###
###########################
class MCAT_Surv_SingleOmic(nn.Module):
    def __init__(self, fusion='concat', omic_size=1, n_classes=4,
                 model_size_wsi='small', model_size_omic='small', dropout=0.25):
        super(MCAT_Surv_SingleOmic, self).__init__()
        self.fusion = fusion
        ...
        ### SNN per il singolo omico
        hidden = self.size_dict_omic[model_size_omic]
        fc_omic = [SNN_Block(dim1=omic_size, dim2=hidden[0])]
        for i in range(len(hidden) - 1):
            fc_omic.append(SNN_Block(dim1=hidden[i], dim2=hidden[i + 1], dropout=0.25))
        self.omic_net = nn.Sequential(*fc_omic)

        ### Multihead Attention
        self.coattn = MultiheadAttention(embed_dim=256, num_heads=1)

        ### Path Transformer + Attention
        ...
        ### Classifier
        self.classifier = nn.Linear(size[2], 4) # quantile bins number

    def forward(self, x_path, x_omic):
        # print(self.n_classes)
        h_path_bag = self.wsi_net(x_path).unsqueeze(1)  # Path embeddings
        h_omic = self.omic_net(x_omic).unsqueeze(0)  # Omic embeddings

        # Co-attention
        h_path_coattn, A_coattn = self.coattn(h_omic, h_path_bag, h_path_bag)
...
class MCAT_Surv(nn.Module):
    ...
    q --> genomic
    '''
    def __init__(self, fusion='concat', omic_sizes=[91, 353, 553, 480, 1566, 480], n_classes=4,
        ...
        ### Classifier
        self.classifier = nn.Linear(size[2], 4)
