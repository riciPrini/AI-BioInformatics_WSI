...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        # slide_data = pd.read_csv(csv_path, compression='zip', header=0, index_col=0, sep=',',  low_memory=False)
        ...
        print("########## slide_data.columns ############")
        # print(slide_data.columns)
        # new_cols = list(slide_data.columns[-2:]) + list(slide_data.columns[:-2])  #original code
        new_cols = list(slide_data.columns[-2:]) + list(slide_data.columns[:-2])
        print("########## new_cols ############")
        # cols_reordered = [col for col in new_cols if col != 'BRCA1_rnaseq'] + ['BRCA1_rnaseq']
        # print(cols_reordered)
        # slide_data = slide_data[cols_reordered]
        ...
        if self.apply_sig:
            self.signatures = pd.read_csv('/work/ai4bio2024/brca_surv/dataset/signatures.csv')
        else:
...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        # print(patients_df["Survival"].head())
        patients_df = slide_data.drop_duplicates(['case_id']).copy()
        uncensored_df = patients_df.loc[patients_df['censorship'] < 1]
        # uncensored_df = patients_df[patients_df['censorship'] < 1]

        # print(uncensored_df["Survival"])
        disc_labels, q_bins = pd.qcut(uncensored_df[label_col], q=n_bins, retbins=True, labels=False)
        ...
        slide_data = slide_data[new_cols]
        self.slide_data = slide_data
        # print("ciao",len(slide_data.columns))
        self.metadata = slide_data.columns[:12] #That was 12, but we have just 11
        # print(self.metadata)
            ...
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    for slide_id in slide_ids:
                        ...
                        wsi_bag = torch.load(wsi_path)
                        path_features.append(wsi_bag)
                    # print("########## Genomic Features ############à")
                    # print("SOno io",self.genomic_features)
                    # print("col: ",self.genomic_features.columns)
                    path_features = torch.cat(path_features, dim=0)
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, mode, signatures=None, data_dir=None, label_col=None, patient_dict=None, num_classes=1):
        self.use_h5 = False
        self.slide_data = slide_data
        ...
        # print("ciao genomic",self.genomic_features)
        ...
        if self.signatures is not None:
            # print("aiuto",self.genomic_features.columns)
            self.omic_names = []
    ...
    def get_scaler(self):
        # print("ciao",self.genomic_features)
