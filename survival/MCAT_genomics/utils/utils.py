...
def ce_loss(hazards, S, Y, c, alpha=0.4, eps=1e-7):
    ...
    #h[y] = h(1)
    #S[1] = S(1)
    # Y = torch.clamp(Y,0,hazards.shape[1] - 1) #questo non va bene perchè forza sempre a 0
    # print(f"hazards.shape: {hazards.shape}, Y.shape: {Y.shape}")
    # print(f"Y: {Y[0]}, hazard : {hazards}")
