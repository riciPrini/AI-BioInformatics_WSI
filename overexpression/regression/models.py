...
class ABMIL(nn.Module):
    def __init__(self,
        ...
        self.fc1 = nn.Linear(inner_dim, inner_dim//2)# -> 64
        self.fc2 = nn.Linear(inner_dim//2, inner_dim//4)# -> 64
        self.fc3 = nn.Linear(inner_dim//4, inner_dim//8)# -> 32
        self.fc4 = nn.Linear(inner_dim//8, inner_dim//16)# -> 16
        self.fc5 = nn.Linear(inner_dim//16, inner_dim//32)# -> 8
        self.fc6 = nn.Linear(inner_dim//32, inner_dim//64)# -> 4
        self.fc7 = nn.Linear(inner_dim//64, output_dim)# -> 1
...
class BClassifier(nn.Module):
    def __init__(self, input_size = 1024, output_dim = 1, inner_dim = 128, dropout_v=0.0, nonlinear=True, passing_v=True): # K, L, N
        ...
        self.fc2 = nn.Linear(inner_dim//2, inner_dim//4)
    ...
    def forward(self, feats, c):
        ...
        x = torch.relu(self.fc1(B)) # relu o tanh
        x = torch.relu(self.fc2(x))
        output = self.fc3(x)
